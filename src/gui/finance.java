...
public class finance extends javax.swing.JPanel {
    ...
    public finance(Home HO) {
        ...
        loadTransaction();
        loadSupplierPayment();
        showPieChartDistributor();
        showPieChartSupplier();
        startAutoRefresh();
        totaldistributors = calculatedistributor();
        totaloutlets = calculateoutlet();
        totalsuppliers = calculatesupplier();
        updateCountsDisplay(totaldistributors, totaloutlets, totalsuppliers);
    ...
    private int calculatedistributor() {
        ...
        try {
            ...
            String query = "SELECT SUM(amount) FROM distributor_payment WHERE payment_status_id = 1";
    ...
    private int calculateoutlet() {
        ...
        try {
            ...
            String query = "SELECT SUM(total_amount) FROM `return_invoice` WHERE `return_invoice_status_id` =1 ";
            rs = MySQL.executeSearch(query);
    ...
    private int calculatesupplier() {
        int totalAmount = 0;
        ResultSet rs = null;

        try {

            String query = "SELECT SUM(paid_amount) FROM `w_grn` WHERE `payment_status_id` =1 ";
            rs = MySQL.executeSearch(query);
    ...
    private void updateCountsDisplay(int DistributorTotalAmount, int OutletTotalAmount, int Suppliertotal) {
        jButton12.setText("<html><div style='text-align: center;'><span style='font-size:11px;'><b>Distributors Pending Payments </b></span>"
                + "<span style='font-size:14px;color:red;'>Rs." + DistributorTotalAmount + "</span><br></br></div></html>");
        jButton16.setText("<html><div style='text-align: center;'><span style='font-size:11px;'><b>Return Pending Payments</b></span>"
                + "<span style='font-size:14px;color:red;'><br>Rs." + OutletTotalAmount + "</span><br></br></div></html>");
        jButton15.setText("<html><div style='text-align: center;'><span style='font-size:11px;' ><b>Supplier Pending Payments</b></span>"
                + "<span style='font-size:14px; color:red;'><br>Rs." + Suppliertotal + "</span><br></br></div></html>");
...
//    }
    ...
    private void loadSupplierPayment() {
        ...
        try {
            ResultSet resultSet = MySQL.executeSearch("SELECT * FROM `supplier_payment` "
                    + " INNER JOIN `w_grn` ON `supplier_payment`.`w_grn_id` = `w_grn`.`id`"
                    + " INNER JOIN `payment_status` ON `supplier_payment`.`payment_status_id`= `payment_status`.`id`"
                    + " INNER JOIN `payment_method` ON `supplier_payment`.`payment_method_id`=`payment_method`.`id` "
                    + "WHERE `payment_status`.`status` = 'Complete'"
                    + "ORDER BY `supplier_payment`.`id` ASC  ");
            ...
            while (resultSet.next()) {
                ...
                vector.add(resultSet.getString("payment_status.status"));
                vector.add(resultSet.getString("payment_method.type"));
                vector.add(resultSet.getString("w_grn_id"));
    ...
    private void printSupplierPaymentSlip() {
        String grnID = jTextField7.getText();
        String amount = jTextField8.getText();
        String payStatus = String.valueOf(jComboBox6.getSelectedItem());
        String payMethod = String.valueOf(jComboBox5.getSelectedItem());
        Date paymentDate = jDateChooser4.getDate();

        // Example data: you can add actual supplier name and ID fields if needed
        String supplierName = "Supplier Name"; // Replace with actual supplier name if available
        String supplierID = grnID;

        if (grnID.isEmpty() || amount.isEmpty() || paymentDate == null || payStatus.equals("Select") || payMethod.equals("Select")) {
            JOptionPane.showMessageDialog(this, "Can't Print Empty or Incomplete Data", "Error", JOptionPane.WARNING_MESSAGE);
            return;
        }

        JFileChooser dialog = new JFileChooser();
        dialog.setSelectedFile(new File(grnID + "_SupplierPaymentSlip.pdf"));
        ...
        if (dialogResult == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                // Title
                Paragraph title = new Paragraph("Supplier Payment Slip (System)", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 20));
                title.setAlignment(Element.ALIGN_CENTER);
                document.add(title);

                // Print date
                Paragraph currentDate = new Paragraph("Date: " + new Date().toString(), FontFactory.getFont(FontFactory.HELVETICA, 10));
                ...
                // Supplier Info
                Paragraph outletInfo = new Paragraph("Supplier Details", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14));
                document.add(outletInfo);
                document.add(Chunk.NEWLINE);
                ...
                outletTable.addCell("Supplier Name:");
                outletTable.addCell(supplierName);
                outletTable.addCell("Supplier ID:");
                outletTable.addCell(supplierID);
                ...
                Paragraph thankYou = new Paragraph("Payment Recorded Successfully!", FontFactory.getFont(FontFactory.HELVETICA_OBLIQUE, 12));
                thankYou.setAlignment(Element.ALIGN_CENTER);
                document.add(thankYou);

                document.close();

                JOptionPane.showMessageDialog(null, "Payment Slip successfully generated!");
    ...
    public void showPieChartDistributor() {
    try {
        DefaultPieDataset pieDataset = new DefaultPieDataset();

        // Query to fetch pending payment amounts by distributor
        String query = "SELECT d.name AS distributor_name, SUM(dp.amount) AS total_pending " +
                       "FROM distributor_payment dp " +
                       "INNER JOIN distributor d ON dp.distributor_id = d.id " +
                       "WHERE dp.payment_status_id = 1 " + // Only Pending payments
                       "GROUP BY d.name";
        ...
        while (rs.next()) {
            ...
            pieDataset.setValue(distributorName, totalPending);
        }

        // Create pie chart
        JFreeChart pieChart = ChartFactory.createPieChart(
                "Distributor Pending Payments",
                pieDataset,
                true,
                true,
                false
        ...
        // Create ChartPanel and add to panel
    ...
    public void showPieChartSupplier() {
    try {
        DefaultPieDataset pieDataset = new DefaultPieDataset();

        // Query to fetch pending payment amounts by supplier
        String query = "SELECT s.company AS supplier_name, SUM(wg.paid_amount) AS total_pending " +
                       "FROM w_grn wg " +
                       "INNER JOIN supplier s ON wg.supplier_id = s.id " +
                       "WHERE wg.payment_status_id = 1 " +  // Pending payments only
                       "GROUP BY s.company";
        ...
        while (rs.next()) {
            ...
            pieDataset.setValue(supplierName, totalPending);
        }
    ...
    }, 0, 5000); // Update every 10 seconds (adjust as needed)
}
    ...
    private void initComponents() {
        ...
        jPanel5.add(jTextField11, new org.netbeans.lib.awtextra.AbsoluteConstraints(780, 30, 140, 30));

        jLabel19.setFont(new java.awt.Font("Trebuchet MS", 0, 18)); // NOI18N
        jLabel19.setText("Search");
        jPanel5.add(jLabel19, new org.netbeans.lib.awtextra.AbsoluteConstraints(720, 30, -1, 30));
        ...
        jTable2.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Date", "Amount", "Method", "Status", "Grn ID"
            }
        ) {
            boolean[] canEdit = new boolean [] {
                false, false, false, false, false, false
        ...
        jPanel6.add(jButton13, new org.netbeans.lib.awtextra.AbsoluteConstraints(668, 42, 210, 46));
        ...
        jPanel6.add(jButton14, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 40, 210, 46));

        jButton17.setFont(new java.awt.Font("Segoe UI Historic", 0, 14)); // NOI18N
        jButton17.setText("Return Payment Invoice");
        jButton17.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton17ActionPerformed(evt);
            }
        });
        jPanel6.add(jButton17, new org.netbeans.lib.awtextra.AbsoluteConstraints(380, 40, 210, 46));
        ...
        jPanel6.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 120, 350, 330));
        ...
        jPanel6.add(jPanel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(550, 120, 360, 320));
        ...
        jButton12.setText("Distributor Pending Payments");
        ...
        jButton16.setText("Returns Pending Total");
    ...
    private void jButton10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton10ActionPerformed
        ...
        if (paymentID.isEmpty() || vehicleNo.isEmpty() || distributorName.isEmpty() || paymentDate == null || amount.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Wrong or Empty Details Entered", "Error", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (payStatus.equals("Select")) {
            JOptionPane.showMessageDialog(this, "Select Payment Status", "Error", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (payMethod.equals("Select")) {
            JOptionPane.showMessageDialog(this, "Select Payment Method", "Error", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void jButton20ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton20ActionPerformed
        ...
        try {
            ...
            jTextField2.setEditable(false);
            ResultSet resultSet = MySQL.executeSearch(
                    "SELECT * FROM `w_grn` WHERE `id` = '"+GrnId+"' "
            );
    ...
    private void jButton21ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton21ActionPerformed
        ...
        try {
            String GrnID = jTextField7.getText().trim();
            String Amounttxt = jTextField8.getText().trim();
            ...
            // Validate that GRN ID exists
            ResultSet rs = MySQL.executeSearch("SELECT id FROM w_grn WHERE id = '" + GrnID + "'");
            if (!rs.next()) {
                JOptionPane.showMessageDialog(this, "Invalid GRN ID. No such record found in w_grn.", "Error", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            // Insert payment
            MySQL.executeIUD(
                    "INSERT INTO `supplier_payment` (`amount`, `date`, `payment_method_id`, `payment_status_id`, `w_grn_id`) "
                    + "VALUES ('" + Amounttxt + "', '" + sdf.format(paymentDate) + "', '"
                    + paymentmethodMap.get(paymentMethod) + "', '"
                    + paymentstatusMap.get(paymentstatus) + "', '" + GrnID + "')"
            );

            // Update GRN status
            MySQL.executeIUD("UPDATE `w_grn` SET `payment_status_id` = 2 WHERE `id` = '" + GrnID + "'");
            ...
            // Load & reset
            loadSupplierPayment();
            printSupplierPaymentSlip(); // You must define this function.
            reset();
    ...
    private void jButton17ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton17ActionPerformed
         ...
         setVisible(true);
        ReturnPaymentInvoice object = new ReturnPaymentInvoice();
        object.setVisible(true);
