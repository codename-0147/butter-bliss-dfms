...
public class distributorpayment extends javax.swing.JFrame {
    ...
    public void deleteDistributorpayment() {
        ...
        if (confirmDelete == JOptionPane.YES_OPTION) {
            try {
                ...
                int rowsAffected = MySQL.executeIUD("DELETE FROM distributor_payment WHERE id = '" + DistributorpayID + "'"
                );
    ...
    private void loadDistributorPayment() {
        ...
        try {
            ResultSet resultSet = MySQL.executeSearch("SELECT * FROM `distributor_payment`"
                    ...
                    + " INNER JOIN `payment_method` ON `distributor_payment`.`payment_method_id`=`payment_method`.`id` ORDER BY `payment_status`.`id` ASC  ");
    ...
    private void initComponents() {
        ...
        jPanel4.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 30, 170, 40));
        ...
        jPanel4.add(jTextField1, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 30, 278, 40));
        ...
        getContentPane().add(jPanel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 90, 560, 690));
        ...
        jPanel2.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 60, 700, 610));
        ...
        jButton7.setIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/icons8-delete-38.png"))); // NOI18N
        jButton7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton7ActionPerformed(evt);
            }
        });
        jPanel2.add(jButton7, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 5, 60, 40));
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        try {
            ...
            if (DistributorID.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(this, "Please Select Pending as the Payment Status", "Error", JOptionPane.WARNING_MESSAGE);
...
//            }
    ...
    Date paymentDate = jDateChooser1.getDate();
    String description = jTextField3.getText();
    String amount = jTextField4.getText();  // You might want a different text field if this duplicates vehicleNo
    String payStatus = (String) jComboBox1.getSelectedItem();
    String payMethod = (String) jComboBox2.getSelectedItem();

        if (name==null || vehicleNo == null || address ==null || salary == null || paymentDate== null || description==null || amount == null || "Select".equals(payStatus) || "Select".equals(payMethod)) {
            JOptionPane.showMessageDialog(this, "Nothing to Print", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
    
    JFileChooser dialog = new JFileChooser();
    dialog.setSelectedFile(new File(vehicleNo+"DistributorPaymentSlip.pdf"));
    ...
    if (dialogResult == JFileChooser.APPROVE_OPTION) {
        ...
        try {
            ...
            distTable.addCell("Distributor Name:");
            distTable.addCell(name);
            distTable.addCell("Vehicle Number:");
            distTable.addCell(vehicleNo);
            distTable.addCell("Address:");
            distTable.addCell(address);
            ...
            payTable.addCell(salary);
            payTable.addCell("Amount Paid:");
            payTable.addCell(amount);
            payTable.addCell("Payment Status:");
            payTable.addCell(payStatus);
            payTable.addCell("Payment Method:");
            payTable.addCell(payMethod);
            payTable.addCell("Payment Date:");
            payTable.addCell(paymentDate != null ? paymentDate.toString() : "N/A");
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if (DistributorID.isEmpty()) {
            ...
            JOptionPane.showMessageDialog(this, "Please Enter The Distributor Name", "Error", JOptionPane.WARNING_MESSAGE);
        } else if (vehicleno.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Wrong Vehicle Number", "Error", JOptionPane.WARNING_MESSAGE);
        } else if (address.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Address is empty", "Error", JOptionPane.WARNING_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Please Enter the description", "Error", JOptionPane.WARNING_MESSAGE);
        } else if (paymentStatus.equals("Select")) {
            JOptionPane.showMessageDialog(this, "Please Select The Payment Status", "Error", JOptionPane.WARNING_MESSAGE);
        } else if (paymentMethod.equals("Select")) {
            JOptionPane.showMessageDialog(this, "Please Select The Payment method", "Error", JOptionPane.WARNING_MESSAGE);
            ...
            ResultSet resultSet = MySQL.executeSearch("SELECT * FROM `distributor_payment` WHERE `id` = '" + DistributorpayID + "' ");
            ...
            if (canUpdate) {
                MySQL.executeIUD("UPDATE `distributor_payment` SET `amount` = '" + salary + "', `payment_date` = '" + sdf.format(paymentDate) + "', `description` = '" + description + "' , `payment_status_id` = '" + paymentstatusMap.get(paymentStatus) + "' , `payment_method_id` = '" + paymentmethodMap.get(paymentMethod) + "'"
                        + "WHERE `id` = '" + DistributorpayID + "' ");
