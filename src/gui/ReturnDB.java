...
private void loadReturnInvoice() {
    try {
        ResultSet resultSet = MySQL.executeSearch(
            ...
            "INNER JOIN collect_return_stock_status ON return_invoice.collect_return_stock_status_id = collect_return_stock_status.id"
        );
...
final String senderPassword = "qyzyofwbgslpexqv"; 
    ...
    private void initComponents() {
        ...
        jPanel2.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(1000, 10, -1, -1));
        ...
        jTable2.setBackground(new java.awt.Color(240, 255, 240));
        ...
        jTable2.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Return Invoice ID", "Date", "Outlet Name", "Reason", "Vehicle No.", "Total Amount", "Status"
            }
        ) {
                    ...
                    .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                        .addGroup(jPanel4Layout.createSequentialGroup()
                            ...
                            .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                ...
                                .addComponent(jTextField9, javax.swing.GroupLayout.DEFAULT_SIZE, 331, Short.MAX_VALUE)
                                .addComponent(jTextField10, javax.swing.GroupLayout.DEFAULT_SIZE, 331, Short.MAX_VALUE)))
                        .addGroup(jPanel4Layout.createSequentialGroup()
        ...
        jPanel2.add(jTabbedPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 60, 1030, 470));
    ...
    try {
        ...
        ResultSet resultSet = MySQL.executeSearch(
            "SELECT " +
            "ri.id AS return_id, " +
            "ri.date AS return_date, " +
            "o.name AS outlet_name, " +
            "d.name AS distributor_name, " +
            "d.vehicle_no AS vehicle_number, " +
            "r.reason AS return_reason, " +
            "ri.total_amount AS total, " +
            "ris.name AS return_status, " +
            "crs.name AS collect_status " +
            "FROM return_invoice ri " +
            "INNER JOIN outlet o ON ri.outlet_id = o.id " +
            "INNER JOIN distributor d ON ri.distributor_id = d.id " +
            "INNER JOIN reason r ON ri.reason_id = r.id " +
            "INNER JOIN return_invoice_status ris ON ri.return_invoice_status_id = ris.id " +
            "INNER JOIN collect_return_stock_status crs ON ri.collect_return_stock_status_id = crs.id " +
            "WHERE ri.date = '" + date + "'"
        );
        ...
        while (resultSet.next()) {
            ...
            row.add(resultSet.getString("return_id"));
            row.add(resultSet.getString("return_date"));
            row.add(resultSet.getString("outlet_name"));
            row.add(resultSet.getString("distributor_name"));
            row.add(resultSet.getString("vehicle_number"));
            row.add(resultSet.getString("return_reason"));
            row.add(resultSet.getString("total"));
            row.add(resultSet.getString("return_status"));
            row.add(resultSet.getString("collect_status"));
            dtm.addRow(row);
        }

        if (!found) {
            JOptionPane.showMessageDialog(this, "No return invoice found for selected date.", "Search Result", JOptionPane.WARNING_MESSAGE);
        }
