...
public void showPieChart() {
    try {
        ...
        // Query to get product names and total revenue
String query = "SELECT wp.name AS product_name, SUM(wii.qty * ws.price) AS total_revenue " +
               "FROM w_invoice_item wii " +
               "INNER JOIN w_stock ws ON wii.w_stock_id = ws.id " +
               "INNER JOIN w_product wp ON ws.w_product_id = wp.id " +
               "GROUP BY wp.name";
        // Execute the query
        ...
        while (rs.next()) {
            ...
            // Add the product name and its total revenue to the pie chart dataset
            pieDataset.setValue(productName, totalRevenue);
        }

        // Create the pie chart
        JFreeChart pieChart = ChartFactory.createPieChart(
                "Product Sales Distribution", // Chart title
                pieDataset,                  // Dataset
                true,                        // Include legend
                true,                        // Tooltips
                false                        // URLs
 ...
 try {
    ...
    private void jTable5MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable5MouseClicked
        if (evt.getClickCount() == 2) {
            ...
            if (selectedRow != -1) {
                ...
                jTextField4.setText(String.valueOf(jTable5.getValueAt(selectedRow,3)));
                jTextField5.setText(String.valueOf(jTable5.getValueAt(selectedRow,0)));
                jTextField6.setText(String.valueOf(jTable5.getValueAt(selectedRow,1)));
                jTextField7.setText(String.valueOf(jTable5.getValueAt(selectedRow,2)));
                jTextField8.setText(String.valueOf(jTable5.getValueAt(selectedRow,4)));
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        try {
            ...
            JOptionPane.showMessageDialog(this, "Supplier Succesfully Deleted");
            loadSupplier();
            reset();
        } catch (Exception e) {
            e.printStackTrace();
